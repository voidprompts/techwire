...
| `node scripts/selftest.mjs` | Offline test suite for the pipeline (24 assertions, no network) |
...
8. **Write** — saves `slug-title.md` with the exact front-matter contract, and records processed URLs in `content/.scraper-state.json`.
...
`title`, `description`, `date`, `keywords`, `image` and `source_url` are the required six; `source_name` and `author` are optional. `keywords` drive the auto-generated topic pages and related-article matching. Hand-written files work exactly the same as generated ones — just drop a `.md` in the folder.
