...
// Keep the optional in-body ad for long-form posts only. With the always-on
// below-title and footer units, shorter articles can drift toward the Better
// Ads 30% mobile ad-density ceiling once reserved ad heights and margins are
// counted. The threshold leaves ~1,000-word briefings with a lighter ad load.
export const MIN_WORDS_FOR_IN_ARTICLE_AD = 1000;
...
function readPostFile(filename) {
  ...
  return {
    ...
    author: (data.author || 'TechWire Editorial Desk').toString(),
    content,
 ...
 */
export function shouldRenderInArticleAd(wordCount) {
  return Number(wordCount) >= MIN_WORDS_FOR_IN_ARTICLE_AD;
}
